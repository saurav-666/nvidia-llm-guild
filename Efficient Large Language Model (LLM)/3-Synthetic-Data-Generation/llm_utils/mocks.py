...
def upload_sentiment(file_path):
...
def create_sentiment_lora_customization(**kwargs):
    def _is_valid_model(model):
        return model == 'gpt-8b-000-lora'

    def _is_valid_train_data(data_id):
        return data_id == '8d55c2d8-c124-46bd-bbf6-864e22f2be9f'

    valid_kwargs = {'model', 'name', 'description', 'batch_size', 
                    'adapter_dim', 'training_dataset_file_id', 'epochs'}
    ...
    adapter_dim = kwargs['adapter_dim']
    if adapter_dim != 32:
        return f'{adapter_dim} is not the correct `adapter_dim`.'
    ...
    if not isinstance(epochs, int) or epochs != 3:
        return f'{epochs} is not the correect value for `epochs`.'
...
def create_qa_lora_customization(**kwargs):
    def _is_valid_model(model):
        return model == 'gpt-8b-000-lora'

    def _is_valid_train_data(data_id):
        return data_id == 'f17e25cd-fd08-42b4-a508-12f48985be35'

    def _is_valid_val_data(data_id):
        return data_id == '30655aa3-17de-41b1-8d73-ddd4a3fadded'
